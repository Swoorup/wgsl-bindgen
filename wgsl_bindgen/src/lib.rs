...
//! wgsl_bindgen is not a rendering library and does not offer high level abstractions like a scene graph or material system.
...
fn main() {
    ...
    let options = wgsl_bindgen::WriteOptions::default();

    // Generate the bindings.
    let text = wgsl_bindgen::create_shader_module(&wgsl_source, "shader.wgsl", options).unwrap();
...
fn main() {
    ...
    // Generate the bindings.
    let text = wgsl_bindgen::create_shader_module_embedded(&wgsl_source, options).unwrap();
    std::fs::write("src/shader.rs", text.as_bytes()).unwrap();
}
```
 */
