...
impl State {
  async fn new(window: Window) -> Self {
    ...
    println!("Max binding array elements per stage: {}", limits.max_binding_array_elements_per_shader_stage);
    println!("Max binding array sampler elements per stage: {}", limits.max_binding_array_sampler_elements_per_shader_stage);
    ...
    window.set_title(&format!("wgsl_bindgen Shader Demos - {}", demo_manager.current_demo().name()));
  ...
  fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
    ...
    self.overlay_renderer.render(&mut render_pass, self.show_info);
...
impl ApplicationHandler<()> for App {
  fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
    ...
    let window = event_loop
      .create_window(Window::default_attributes().with_title("wgsl_bindgen Shader Demos"))
      .unwrap();
        ...
        WindowEvent::KeyboardInput { event, .. } => {
          ...
          if event.state == ElementState::Pressed {
            ...
            if state.demo_manager.handle_input(&event) {
              ...
              state.window.set_title(&format!("wgsl_bindgen Shader Demos - {}", state.demo_manager.current_demo().name()));
              // Update overlay text
              state.overlay_renderer.update_help_text(&state.device, &state.queue, state.demo_manager.get_help_text());
