...
impl DemoManager {
    pub fn new(
        ...
        let mut demos: Vec<Box<dyn Demo>> = Vec::new();
        
        // Add all available demos
        demos.push(Box::new(TriangleDemo::new(device, queue, surface_format)));
        demos.push(Box::new(TextureArrayDemo::new(device, queue, surface_format)));
    ...
    pub fn update(&mut self, device: &wgpu::Device, queue: &wgpu::Queue, elapsed_time: f32) {
        self.current_demo_mut().update(device, queue, elapsed_time);
    }

    pub fn render<'a>(&'a mut self, device: &wgpu::Device, render_pass: &mut wgpu::RenderPass<'a>) {
